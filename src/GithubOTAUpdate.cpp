 ...
 * @file GithubOTAUpdate.cpp based on HTTPUpdate.cpp
 * @date 24.10.2023
...
GithubOTAUpdate::GithubOTAUpdate(void)
  : _httpClientTimeout(8000), _ledPin(-1) {
  _followRedirects = HTTPC_DISABLE_FOLLOW_REDIRECTS;
}

GithubOTAUpdate::GithubOTAUpdate(int httpClientTimeout)
...
GithubOTAUpdateResult GithubOTAUpdate::update(WiFiClient& client, const String& url, const String& currentVersion, GithubOTAUpdateRequestCB requestCB) {
  HTTPClient http;
  if (!http.begin(client, url)) {
    return GithubOTA_UPDATE_FAILED;
  }
  return handleUpdate(http, currentVersion, false, requestCB);
}

GithubOTAUpdateResult GithubOTAUpdate::updateSpiffs(HTTPClient& httpClient, const String& currentVersion, GithubOTAUpdateRequestCB requestCB) {
  return handleUpdate(httpClient, currentVersion, true, requestCB);
}

GithubOTAUpdateResult GithubOTAUpdate::updateSpiffs(WiFiClient& client, const String& url, const String& currentVersion, GithubOTAUpdateRequestCB requestCB) {
  HTTPClient http;
  if (!http.begin(client, url)) {
    return GithubOTA_UPDATE_FAILED;
  }
...
GithubOTAUpdateResult GithubOTAUpdate::update(HTTPClient& httpClient,
                                    const String& currentVersion, GithubOTAUpdateRequestCB requestCB) {
  return handleUpdate(httpClient, currentVersion, false, requestCB);
}

GithubOTAUpdateResult GithubOTAUpdate::update(WiFiClient& client, const String& host, uint16_t port, const String& uri,
                                    const String& currentVersion, GithubOTAUpdateRequestCB requestCB) {
  HTTPClient http;
 ...
 */
int GithubOTAUpdate::getLastError(void) {
  return _lastError;
}
 ...
 * @return String error
 */
String GithubOTAUpdate::getLastErrorString(void) {
 ...
 * @return GithubOTAUpdateResult
 */
GithubOTAUpdateResult GithubOTAUpdate::handleUpdate(HTTPClient& http, const String& currentVersion, bool spiffs, GithubOTAUpdateRequestCB requestCB) {

  GithubOTAUpdateResult ret = GithubOTA_UPDATE_FAILED;
  ...
  if (!_githubToken.isEmpty()) {
    Serial.println("Got the githubToken  "); 
    http.addHeader(F("Accept"), "application/vnd.github+json"); 
    http.addHeader(F("Authorization"), "Bearer " + _githubToken);
    http.addHeader(F("X-GitHub-Api-Version"), "2022-11-28");
  }
...
*/
    ...
    case HTTP_CODE_NOT_MODIFIED:
      ...
      ret = GithubOTA_UPDATE_NO_UPDATES;
      break;
 ...
 */
bool GithubOTAUpdate::runUpdate(Stream& in, uint32_t size, String md5, int command) {
...
GithubOTAUpdate githubOTAUpdate;
#endif
